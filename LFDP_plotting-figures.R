...
tiff ('LFDP_3D_view_onlyone.tiff', width = 9, height = 9, units = 'cm', res = 600, compression = 'lzw', pointsize = 6)
require (plot3D)
#par (mfrow = c(1,2))
persp3D (x = IL$x, y = IL$y, z = IL$z, col = ramp.col(c("white", "green")), shade = .6,
         phi = 30, theta = 30, resfac = 1, 
         scale = TRUE, expand = .4, border = 'gray', zlim = c(1740, 1790),
         contour = list(nlevels = 12, col = "black", side = c("z"), lwd = .5),
         image = FALSE, box = TRUE, clab = "Elevation\n[m asl]",
         xlab = expression ("West-East"), ylab = "South-North", zlab = 'Elevation',
         cex.main = 2, colkey = FALSE)
...
g_veg <- ggplot (xy_veg, aes (x = x, y = y)) +
  #geom_contour (data = pile[1:121,], aes (x = I(x*10), y = I(y*10), z = elevation), linewidth = .5, colour = 'grey', linejoin = 'mitre', show.legend = FALSE) + 
  geom_contour (data = IL_df, aes (x = I(x*10), y = I(y*10), z = z), linewidth = 0.5, colour = 'grey', linejoin = 'mitre', show.legend = FALSE) + 
  ...
  theme (panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
         ...
         legend.key.size = unit(0.3, "cm"), legend.title = element_text(size = 6.5)) +
  coord_fixed(ratio = 1) + 
  scale_x_continuous(expand=c(0.01, 0.01), breaks = seq(0, 100, by = 10)) +
  scale_y_continuous(expand=c(0.01, 0.01), breaks = seq(0, 100, by = 10)) +
...
tiff ('LFDP-vegtypes.tiff', width = 8, height = 9, units = 'cm', res = 600, compression = 'lzw')
g_veg
dev.off ()
...
orditorp (DCA, display = 'sp', labels = spe_names, col = 'grey20', cex = .6, priority = table (ind_agg$species), pch = 3, pcex = .3, pcol = 'grey70', font = 3)
title (main = 'B', adj = 0)
dev.off ()
